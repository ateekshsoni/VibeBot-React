    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center text-green-400 text-sm">
                  ...
                  +{audienceData.overview.newFollowers.toLocaleString()} this month
                </div>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{audienceData.overview.engagementRate}%</p>
                <div className="flex items-center text-green-400 text-sm">
                  <ArrowUp className="h-3 w-3 mr-1" />
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  {((audienceData.overview.activeFollowers / audienceData.overview.totalFollowers) * 100).toFixed(1)}% of total
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{audienceData.overview.averageAge}</p>
                <p className="text-sm text-gray-400">
                  {audienceData.overview.genderSplit.female}% F • {audienceData.overview.genderSplit.male}% M
                </p>
              </div>
            ...
            <div className="space-y-4">
              {audienceData.demographics.ageGroups.map((group, index) => (
                <div key={index} className="flex items-center space-x-3">
                  <div className="w-16 text-sm text-gray-300">{group.range}</div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="text-white">{group.percentage}%</span>
                      <span className="text-gray-400">{group.count.toLocaleString()}</span>
                    </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {audienceData.demographics.topLocations.map((location, index) => (
                <div key={index} className="flex items-center space-x-3">
                  <div className="w-20 text-sm text-gray-300">{location.country}</div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between text-sm mb-1">
            ...
            <div className="grid grid-cols-6 gap-2">
              {audienceData.behavior.activeHours.map((hour, index) => (
                <div key={index} className="text-center">
                  <div className="text-xs text-gray-400 mb-1">{hour.hour}:00</div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">{hour.activity}%</div>
                </div>
              ))}
            </div>
          </CardContent>
            ...
            <div className="space-y-3">
              {audienceData.behavior.topInterests.map((interest, index) => (
                <div key={index} className="flex items-center justify-between">
                  <span className="text-gray-300 text-sm">{interest.interest}</span>
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-white text-sm w-8">{interest.percentage}%</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        <CardHeader>
          ...
          <CardDescription>How different content types perform with your audience</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {audienceData.behavior.contentPreferences.map((content, index) => {
              const getIcon = (type) => {
                switch (type) {
                  case 'Photos': return <Camera className="h-6 w-6" />;
                  case 'Videos': return <PlayCircle className="h-6 w-6" />;
                  case 'Carousels': return <Share2 className="h-6 w-6" />;
                  case 'Stories': return <MessageCircle className="h-6 w-6" />;
                  case 'Reels': return <PlayCircle className="h-6 w-6" />;
              ...
              return (
                <div key={index} className="bg-gray-800 p-4 rounded-lg text-center">
                  ...
                  <h4 className="text-white font-medium text-sm mb-1">{content.type}</h4>
                  <p className="text-2xl font-bold text-white mb-1">{content.engagement}%</p>
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        <CardHeader>
          ...
          <CardDescription>Different groups within your audience</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {audienceData.segments.map((segment, index) => (
              <div key={index} className="bg-gray-800 p-4 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <div className={`p-2 rounded-lg bg-gradient-to-r ${getSegmentColor(segment.color)} text-white`}>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-white font-medium">{segment.avgEngagement}%</p>
                  </div>
                </div>
