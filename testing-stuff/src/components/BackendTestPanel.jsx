    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Backend Integration Tests</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700">User Sync</span>
              <div className={`w-3 h-3 rounded-full ${isBackendSynced ? 'bg-green-500' : syncLoading ? 'bg-yellow-500' : 'bg-red-500'}`}></div>
            </div>
            <span className="text-xs text-gray-500">
              {isBackendSynced ? 'Synced' : syncLoading ? 'Syncing...' : 'Not synced'}
            </span>
          </div>
        ...
        <div className="space-y-3">
          ...
          <TestResult testKey="instagramStatus" label="Instagram Status Endpoint" />
          <TestResult testKey="dashboardOverview" label="Dashboard Overview Endpoint" />
          <TestResult testKey="userAnalytics" label="User Analytics Endpoint" />
        </div>

        {/* Manual Actions */}
        <div className="mt-6 pt-6 border-t">
          <h3 className="text-lg font-semibold text-gray-900 mb-3">Manual Actions</h3>
        ...
        {/* Environment Info */}
        <div className="mt-6 pt-6 border-t">
          <h3 className="text-lg font-semibold text-gray-900 mb-3">Environment Configuration</h3>
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <span className="font-medium text-gray-700">API URL:</span>
                <span className="text-gray-600 ml-2">{import.meta.env.VITE_API_URL}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Environment:</span>
                <span className="text-gray-600 ml-2">{import.meta.env.VITE_APP_ENV}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Frontend URL:</span>
                <span className="text-gray-600 ml-2">{import.meta.env.VITE_FRONTEND_URL}</span>
              </div>
              <div>
