...
const DmAutoReplyConfig = ({ isOpen, onClose, onSave }) => {
  const [autoReplies, setAutoReplies] = useState([
    {
      ...
      response: "Hey there! 👋 Welcome to our community! How can I help you today?",
      delay: 0,
      isActive: true,
      triggerCount: 156,
      priority: 1
    },
    {
      ...
      response: "Thanks for reaching out! 🕐 Our team typically responds within 2-4 hours during business hours (9AM-6PM EST). For urgent matters, please mention 'URGENT' in your message.",
      delay: 30,
      isActive: true,
      triggerCount: 89,
      priority: 2
    },
    {
      ...
      response: "Great question! 💰 Our pricing varies based on your needs. I'll connect you with our team who can provide personalized pricing. In the meantime, check out our packages: [Link]",
      delay: 15,
      isActive: false,
      triggerCount: 67,
      priority: 3
  ...
  const addAutoReply = () => {
    if (newReply.name.trim() && newReply.triggers.trim() && newReply.response.trim()) {
      const triggers = newReply.triggers.split(',').map(t => t.trim()).filter(t => t);
      
      setAutoReplies(prev => [...prev, {
        id: Date.now(),
        name: newReply.name,
        triggers,
      ...
      setNewReply({ name: '', triggers: '', response: '', delay: 0, isActive: true, priority: 1 });
    }
  };
  ...
  const updatePriority = (id, direction) => {
    setAutoReplies(prev => {
      ...
      [newReplies[targetIndex], newReplies[currentIndex]];
      
      return newReplies.map((reply, index) => ({ ...reply, priority: index + 1 }));
    });
  };
        ...
        <div className="p-6 border-b border-gray-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">DM Auto-Reply Configuration</h2>
                <p className="text-gray-400 text-sm">Set up intelligent automatic responses to direct messages</p>
              </div>
            </div>
        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-200px)]">
              ...
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">Total Responses (30d)</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gray-800/50 border-gray-700">
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-white">
                      {Math.round(autoReplies.reduce((sum, r) => sum + r.delay, 0) / autoReplies.length)}s
                    </p>
          ...
          <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <p className="text-green-400 font-medium">Smart Auto-Reply System</p>
                <p className="text-gray-300 text-sm mt-1">
                  Responses are processed in priority order. First matching trigger wins. Add delays to make responses feel more natural.
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 space-y-3">
                      ...
                      <div>
                        <h4 className="text-white font-medium mb-2">{reply.name}</h4>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {reply.triggers.map((trigger, triggerIndex) => (
                            <Badge key={triggerIndex} variant="outline" className="text-xs bg-green-500/10 text-green-400 border-green-500/30">
                              "{trigger}"
                            </Badge>
                          ))}
                        </div>
                      ...
                      <div>
                        <p className="text-sm text-gray-300 font-medium mb-2">Auto-Response:</p>
                        <div className="bg-gray-800 p-3 rounded border-l-4 border-green-500">
                          <p className="text-sm text-gray-300">{reply.response}</p>
                        </div>
                      </div>

                      {showPreview && (
                        <div className="bg-gray-900 p-3 rounded border border-gray-600">
                          <p className="text-xs text-gray-400 mb-2">Preview: How users see the response</p>
          ...
          <Card className="bg-gray-800/30 border-gray-700 border-dashed">
            <CardHeader>
              ...
              <CardDescription>
                Create intelligent responses that trigger automatically based on message content
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewReply(prev => ({ ...prev, delay: e.target.value }))}
                    placeholder="0"
                    min="0"
                    max="300"
                    className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewReply(prev => ({ ...prev, triggers: e.target.value }))}
                  placeholder="hello, hi, start, welcome"
                  className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500"
                />
                <p className="text-xs text-gray-500 mt-1">
                  Messages containing any of these words will trigger this response
                </p>
              </div>
              ...
              <Button 
                ...
                disabled={!newReply.name.trim() || !newReply.triggers.trim() || !newReply.response.trim()}
                ...
                {[
                  {
                    ...
                    response: "Thanks for reaching out! 🕐 I'm currently away but will respond within 24 hours. For urgent matters, please email support@company.com",
                    delay: 60
                  },
                  {
                    name: "Product Inquiry",
                    triggers: "product, features, demo",
                    response: "Great question! 🚀 I'd love to show you our features. Let me send you a quick demo link: [Demo Link]",
                    delay: 30
                  },
                  {
                    name: "Thank You",
                    triggers: "thanks, thank you, appreciate",
                    response: "You're so welcome! 😊 I'm here if you need anything else. Have an amazing day!",
                    delay: 0
                  },
                  {
                    name: "Consultation Booking",
                    triggers: "book, schedule, consultation, call",
                    response: "Perfect! 📅 I'd love to chat. Here's my calendar link to book a free 15-min consultation: [Calendar Link]",
                    delay: 15
                  }
                ].map((template, index) => (
                  <div 
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-400 mb-1">Delay: {template.delay}s</p>
                    <p className="text-xs text-gray-400 mb-2">Triggers: {template.triggers}</p>
        ...
        <div className="p-6 border-t border-gray-700 bg-gray-800/50">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-400">
              {autoReplies.filter(r => r.isActive).length} active auto-reply rules
            </div>
            <div className="flex items-center space-x-2">
              <Button variant="outline" onClick={onClose}>
                Cancel
