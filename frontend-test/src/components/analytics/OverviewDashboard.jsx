            ...
            {chartData[chartType].map((item, index) => (
              <div key={index} className="flex-1 flex flex-col items-center">
                <div
                  ...
                  style={{
                    height: `${(item.value / Math.max(...chartData[chartType].map(d => d.value))) * 200}px`,
                    minHeight: '20px'
                  }}
                />
                <p className="text-xs text-gray-400 mt-2">{item.day}</p>
                <p className="text-xs text-white font-medium">
                  {chartType === 'engagement' ? `${item.value}%` : 
                   chartType === 'reach' ? `${(item.value / 1000).toFixed(1)}k` : 
                   `+${item.value}`}
                </p>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <div className={`flex items-center text-xs ${
                    ...
                    metric.trend === 'down' ? 'text-blue-400' : 'text-gray-400'
                  }`}>
        ...
        <Card className="bg-gray-800/50 border-gray-700">
          <CardHeader>
            ...
            <CardDescription>
              Ranked by success rate and impact
            </CardDescription>
          </CardHeader>
            ...
            {topPerformers.map((performer, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-white font-medium text-sm">{performer.automation}</p>
                    <p className="text-xs text-gray-400">
                      {performer.triggers} triggers • {performer.successRate}% success
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Badge 
                    variant={performer.impact === 'High' ? 'default' : 'secondary'}
                    className={performer.impact === 'High' ? 'bg-green-500/20 text-green-400' : 'bg-blue-500/20 text-blue-400'}
                  >
                    {performer.impact}
                  </Badge>
                  {performer.trend === 'up' ? (
