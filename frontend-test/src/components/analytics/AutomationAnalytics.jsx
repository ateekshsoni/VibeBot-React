...
const AutomationAnalytics = ({ data, dateRange }) => {
  ...
  const getFilteredData = () => {
    ...
    return automationData.filter(automation => automation.id === selectedAutomation);
  };

  const totalMetrics = automationData.reduce((acc, automation) => ({
  ...
  totalMetrics.avgSuccessRate = totalMetrics.avgSuccessRate / automationData.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center space-x-2">
        ...
        {automationData.map((automation) => (
          <Button
            key={automation.id}
            size="sm"
            variant={selectedAutomation === automation.id ? 'default' : 'outline'}
            onClick={() => setSelectedAutomation(automation.id)}
            className={selectedAutomation === automation.id ? `bg-${automation.color}-500` : ''}
          >
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{totalMetrics.triggers.toLocaleString()}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{totalMetrics.avgSuccessRate.toFixed(1)}%</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{totalMetrics.conversions}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">${totalMetrics.revenue.toLocaleString()}</p>
              </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-lg bg-gradient-to-r ${automation.gradient} text-white`}>
                    {automation.icon}
                  </div>
                  <div>
                    <CardTitle className="text-lg text-white">{automation.name}</CardTitle>
                    <CardDescription>
                ...
                <Badge 
                  ...
                  className={automation.status === 'active' ? 'bg-green-500/20 text-green-400' : ''}
                >
                  {automation.status}
                </Badge>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-800 p-3 rounded-lg">
                  ...
                  <p className="text-xl font-bold text-white">{automation.triggers.toLocaleString()}</p>
                </div>
                
                <div className="bg-gray-800 p-3 rounded-lg">
                  <span className="text-sm text-gray-400">Success Rate</span>
                  <p className="text-xl font-bold text-white">{automation.successRate}%</p>
                </div>
                
                <div className="bg-gray-800 p-3 rounded-lg">
                  <span className="text-sm text-gray-400">Avg Response</span>
                  <p className="text-xl font-bold text-white">{automation.avgResponseTime}s</p>
                </div>
                
                <div className="bg-gray-800 p-3 rounded-lg">
                  <span className="text-sm text-gray-400">Conversions</span>
                  <p className="text-xl font-bold text-white">{automation.conversions}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-white font-medium">${automation.revenue.toLocaleString()}</span>
                </div>
                
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-400">User Satisfaction:</span>
                  <div className="flex items-center">
                    <span className="text-white font-medium mr-2">{automation.details.userSatisfaction}/5</span>
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                        <div
                          key={i}
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-white font-medium">{automation.details.avgEngagement}%</span>
                </div>
              </div>

              {/* Top Keywords */}
              <div>
                <p className="text-sm font-medium text-gray-300 mb-2">Top Performing Keywords:</p>
              ...
              {/* Peak Hours */}
              <div>
                <p className="text-sm font-medium text-gray-300 mb-2">Peak Activity Hours:</p>
                <div className="flex space-x-2">
                  {automation.details.peakHours.map((hour, index) => (
                    <div key={index} className="bg-gray-800 px-2 py-1 rounded text-xs text-white">
                      {hour}:00
                    </div>
                  ))}
                </div>
      ...
      <Card className="bg-gray-800/50 border-gray-700">
        <CardHeader>
          <CardTitle className="text-lg text-white">Automation Performance Comparison</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Success Rate Chart */}
            <div>
              <h4 className="text-sm font-medium text-gray-300 mb-4">Success Rate Comparison</h4>
              <div className="space-y-3">
                {automationData.map((automation) => (
                  <div key={automation.id} className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between text-sm mb-1">
                        ...
                        <span className="text-gray-400">{automation.successRate}%</span>
                      </div>
            ...
            <div>
              <h4 className="text-sm font-medium text-gray-300 mb-4">Trigger Volume</h4>
              <div className="space-y-3">
                {automationData.map((automation) => {
                  const maxTriggers = Math.max(...automationData.map(a => a.triggers));
                  ...
                  return (
                    <div key={automation.id} className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between text-sm mb-1">
                          <span className="text-white">{automation.name}</span>
                          <span className="text-gray-400">{automation.triggers}</span>
                        </div>
                        <div className="w-full bg-gray-700 rounded-full h-2">
                          <div
                            className={`bg-gradient-to-r ${automation.gradient} h-2 rounded-full transition-all duration-300`}
